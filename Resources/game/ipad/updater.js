		...
		xhr.onload = function(e) { 	
			...
			if(jsonData.RESPONSE == '1'){
				...
				for(var i=0; i < questionsLength; i++){
					...
					db.execute('update questions set category_id=?, question=?, answer_a=?, answer_b=? ,answer_c=?, answer_d=?, correct=?, value=? where question_id=?', categoryId, question, answer_a, answer_b, answer_c, answer_d, correct, value, id);
					rowsAffected = db.getRowsAffected();
					if(rowsAffected == 0){
						db.execute('insert into questions (category_id,question,answer_a, answer_b,answer_c,answer_d,correct,value,question_id) values (?,?,?,?,?,?,?,?,?)', categoryId, question, answer_a, answer_b, answer_c, answer_d, correct, value, id);
						inserts++;
					} else {
						updates++;
					}
