...
//UI positioning
var QUESTION_NEXT_HEIGHT_OFFSET_FOR_LOST_PLAYER = 1;
var QUESTION_NEXT_IPHONE5_OFFSET_HEIGHT = 40;
...
function buildQuestionNextView(){
	Ti.API.info('buildQuestionNextView started! - gameSession.getGameType()= '+gameSession.getGameType());
	...
	if(shouldCreateView){
		//Blue Background
		var blueBack = IPHONE5 ? gameSession.getGameType() == BUZZ_GAME_SOLO ? IMAGE_PATH+'question_next/background_solo-568h.png' : IMAGE_PATH+'question_next/background-568h.png' : gameSession.getGameType() == BUZZ_GAME_SOLO ? IMAGE_PATH+'question_next/background_solo.png' : IMAGE_PATH+'question_next/background.png';
		...
		questionNextLabelPlayerNo = Ti.UI.createLabel({
			...
			top:IPHONE5? 80+QUESTION_NEXT_IPHONE5_OFFSET_HEIGHT : 80,
			textAlign:'center',
			height:80,
			width:180,
			color:'yellow',
			font:{fontSize: 29, fontWeight: 'bold', fontFamily:'321Impact'}
		});
		...
		if(gameSession.getGameType() == BUZZ_GAME_SOLO){
			...
			nextQuestionPointsLabel = Titanium.UI.createLabel({
			    ...
			    zIndex:10,
			    top:IPHONE5? 142+QUESTION_NEXT_IPHONE5_OFFSET_HEIGHT : 142,
			    left:5,
			    right:5,
			...
			questionNextLabelPlayerName = Ti.UI.createLabel({
				...
				top:IPHONE5? 125+QUESTION_NEXT_IPHONE5_OFFSET_HEIGHT : 125,
				textAlign:'center',
				height:70,
				width:250,
				color:'white',
		...
		var nextPlayerButtonTop = IPHONE5 ? 190+QUESTION_NEXT_IPHONE5_OFFSET_HEIGHT : gameSession.getGameType() == BUZZ_GAME_SOLO ? 185 : 185
		questionNextButtonNext = Ti.UI.createButton({
			backgroundImage:questionNextPlayImageButton,
			top:nextPlayerButtonTop,
			width:127,
			height:127
		});
...
function showQuestionNextPopupLost(){
	...
	questionNextPopupLostNameLabel = Ti.UI.createLabel({
		...
		width:200,
		color:'white',
		font:{fontSize: 45, fontWeight: 'regular', fontFamily:'Myriad Pro'}
	});
	...
	questionNextPlayerLostScoreValue = Ti.UI.createLabel({
		...
		width:200,
		color:'#fee902',
		font:{fontSize: IPHONE5? 45:38, fontWeight: 'bold', fontFamily:'321Impact'}
	});
...
function handleQuestionNextWikipedia(e){
	...
	if(clickedButton == 'wikipedia'){
		...
		if(Titanium.Network.online){
			...
			//questionNextWikipediaView.bottom = 0;
		...
		var targetBottom = IPHONE5? 479+82 : 479;
		questionNextWikipediaView.animate({bottom:targetBottom,duration:350});
	}
}
