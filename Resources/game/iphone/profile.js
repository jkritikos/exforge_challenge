var myFacebookImage = IMAGE_PATH+'profile/user_noimage_new120.png';
//The scoreData
var playerData = null;
...
viewProfile.addEventListener('myProfile', function(e) {
	...
	if(currentPlayer.facebook_id == null || currentPlayer.facebook_id == ''){
		myFacebookImage = IMAGE_PATH+'profile/user_noimage_new120.png';
	} else {
		myFacebookImage = 'http://graph.facebook.com/'+fbId+'/picture?height=60&width=60';
	}
...
function buildProfileView(){
	...
	if(shouldCreateView){
		...
		friendImage = Ti.UI.createImageView({
			...
			defaultImage:IMAGE_PATH+'profile/user_noimage_new120.png',
			left:13,
			top:40,
			borderColor:'white',
			borderWidth:2
		...
		topScoreLabelValue = Titanium.UI.createLabel({
			...
			height:40,
			width:65,
			font:{fontSize:26, fontWeight:'bold', fontFamily:'321impact'}
		});
